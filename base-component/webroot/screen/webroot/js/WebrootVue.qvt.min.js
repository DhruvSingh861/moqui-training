...
notificationClient:null,sessionTokenBc:null,qzVue:null,leftOpen:!1,moqui},methods:{setUrl:function(a,b,c){this.currentLoadRequest&&(console.log("Aborting current page load currentLinkUrl "+this.currentLinkUrl+" url "+a),this.currentLoadRequest.abort(),this.currentLoadRequest=null,this.loading=0);this.bodyParameters=b;a=this.getLinkPath(a);if(this.currentLinkUrl===a&&a!==this.linkBasePath)this.reloadSubscreens(),c&&this.callOnComplete(c,this.currentPath);else{var d=this.currentPath;b=moqui.parseHref(a);
this.extraPathList=[];this.currentSearch=b.search;this.currentPath=b.path;b=this.currentSearch;if((b=this.currentPath+(0<b.length?"?"+b:""))&&0!==b.length){console.info("Current URL changing to "+b);this.lastNavTime=Date.now();this.activeContainers={};var e=this;b=this.appRootPath&&this.appRootPath.length&&0===b.indexOf(this.appRootPath)?this.appRootPath+"/menuData"+b.slice(this.appRootPath.length):"/menuData"+b;$.ajax({type:"GET",url:b,dataType:"text",error:moqui.handleAjaxError,success:function(f){var g=
null;try{g=JSON.parse(f)}catch(h){console.info("Error parson menu list JSON: "+h)}g&&moqui.isArray(g)&&(e.navMenuList=g,c&&e.callOnComplete(c,d))}});window.history.pushState(null,this.ScreenTitle,a);this.urlListeners.forEach(function(f){f(a,this)},this);document.documentElement.scrollTop=0;document.body.scrollTop=0}}},callOnComplete:function(a,b){if(a){var c=this.getRoute();b&&(c.redirectedFrom=b);a(c)}},getRoute:function(){return{name:this.currentPathList[this.currentPathList.length-1],meta:{},path:this.currentPath,
hash:"",query:this.currentParameters,params:this.bodyParameters||{},fullPath:this.currentLinkUrl,matched:[]}},setParameters:function(a){if(a){this.$root.currentParameters=$.extend({},this.$root.currentParameters,a);a=this.currentLinkUrl;var b=this.navHistoryList[0];b.pathWithParams=a;window.history.pushState(null,b.title||"",a)}this.$root.reloadSubscreens()},addSubscreen:function(a){a.pathIndex=this.activeSubscreens.length;a.loadActive();this.activeSubscreens.push(a)},reloadSubscreens:function(){var a=
...
reLoginVerifyOtpResponse:function(a,b,c){this.getCsrfToken(c);a.loggedIn&&this.reLoginPostLogin()}},watch:{navMenuList:function(a){if(0<a.length){var b=a[a.length-1];a=1<a.length?a[a.length-2]:null;b.extraPathList&&(this.extraPathList=b.extraPathList);var c=this.basePathSize,d=b.path.split("/").slice(c+1);console.info("nav updated fullPath "+JSON.stringify(d)+" currentPathList "+JSON.stringify(this.currentPathList)+" cur.path "+b.path+" basePathSize "+c);this.currentPathList=d;this.reloadSubscreens();
a=(a?a.title+" - ":"")+b.title;c=b.pathWithParams;var e=c.indexOf("?");if(0<e){d=["pageIndex","orderBySelect","orderByField","moquiSessionToken"];var f=c.substring(e+1).split("\x26");c=c.substring(0,e);e=0;for(var g="",h=0;h<f.length;h++){var n=f[h];c=-1===c.indexOf("?")?c+"?":c+"\x26";c+=n;if(!(3<e)){var m=n.indexOf("\x3d");if(0<m){var l=n.substring(0,m);n=n.substring(m+1);0<l.indexOf("_op")||0<l.indexOf("_not")||0<l.indexOf("_ic")||0<=d.indexOf(l)||l===n||(0<g.length&&(g+=", "),g+=decodeURIComponent(n),
e++)}}}0<g.length&&(70<g.length&&(g=g.substring(0,70)+"..."),a=a+" ("+g+")")}d=this.navHistoryList;for(f=0;f<d.length;)d[f].pathWithParams===c?d.splice(f,1):f++;for(d.unshift({title:a,pathWithParams:c,image:b.image,imageType:b.imageType});25<d.length;)d.pop();document.title=a}},currentPathList:function(a){(a=a[a.length-1])&&$(this.$el).removeClass().addClass(a)}},computed:{currentPath:{get:function(){var a=this.currentPathList,b=this.extraPathList;return this.basePath+(a&&0<a.length?"/"+a.join("/"):
"")+(b&&0<b.length?"/"+b.join("/"):"")},set:function(a){a&&0!==a.length?("/"===a.slice(a.length-1)&&(a=a.slice(0,a.length-1)),0===a.indexOf(this.linkBasePath)?a=a.slice(this.linkBasePath.length+1):0===a.indexOf(this.basePath)&&(a=a.slice(this.basePath.length+1)),this.currentPathList=a.split("/")):this.currentPathList=[]}},currentLinkPath:function(){var a=this.currentPathList,b=this.extraPathList;return this.linkBasePath+(a&&0<a.length?"/"+a.join("/"):"")+(b&&0<b.length?"/"+b.join("/"):"")},currentSearch:{get:function(){return moqui.objToSearch(this.currentParameters)},
set:function(a){this.currentParameters=moqui.searchToObj(a)}},currentLinkUrl:function(){var a=this.currentSearch;return this.currentLinkPath+(0<a.length?"?"+a:"")},basePathSize:function(){return this.basePath.split("/").length-this.appRootPath.split("/").length},ScreenTitle:function(){return 0<this.navMenuList.length?this.navMenuList[this.navMenuList.length-1].title:""},documentMenuList:function(){for(var a=[],b=0;b<this.navMenuList.length;b++){var c=this.navMenuList[b].screenDocList;c&&c.length&&
c.forEach(function(d){a.push(d)})}return a}},created:function(){this.moquiSessionToken=$("#confMoquiSessionToken").val();this.appHost=$("#confAppHost").val();this.appRootPath=$("#confAppRootPath").val();this.basePath=$("#confBasePath").val();this.linkBasePath=$("#confLinkBasePath").val();this.userId=$("#confUserId").val();this.username=$("#confUsername").val();this.locale=$("#confLocale").val();moqui.localeMap[this.locale]&&(this.locale=moqui.localeMap[this.locale]);this.leftOpen="true"===$("#confLeftOpen").val();
var a=$("#confDarkMode").val();this.$q.dark.set("true"===a);this.notificationClient=new moqui.NotificationClient(("https:"===location.protocol?"wss://":"ws://")+this.appHost+this.appRootPath+"/notws");this.sessionTokenBc=new BroadcastChannel("SessionToken");this.sessionTokenBc.onmessage=this.receiveBcCsrfToken;var b=[];$(".confNavPluginUrl").each(function(d,e){b.push($(e).val())});this.addNavPluginsWait(b,0);var c=[];$(".confAccountPluginUrl").each(function(d,e){c.push($(e).val())});this.addAccountPluginsWait(c,
...
beforeDestroy:function(){this.sessionTokenBc.close()}});window.addEventListener("popstate",function(){moqui.webrootVue.setUrl(window.location.pathname+window.location.search)});
