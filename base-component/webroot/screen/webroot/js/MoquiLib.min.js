...
"");return-1===a.indexOf(moqui.decimalSeparator)?parseInt(a):parseFloat(a)},format:function(a,b,c){b&&b.length&&(b=b.replace(/a/,"A").replace(/d/,"D").replace(/y/,"Y"));if(c&&c.length){c=c.toLowerCase();if("date"===c)return b&&0!==b.length||(b="YYYY-MM-DD"),moment(a).format(b);if("time"===c)return b&&0!==b.length||(b="HH:mm:ss"),moment(a).format(b);if("timestamp"===c)return b&&0!==b.length||(b="YYYY-MM-DD HH:mm"),moment(a).format(b);if("bigdecimal"===c||"currency"===c)return moqui.isNumber(a)&&(a=
a.toFixed(2)),(""+a).replace(/(\d)(?=(\d{3})+(?!\d))/g,"$1,");if("long"===c||"integer"===c||"double"===c||"float"===c)return moqui.isString(a)||(a=""+a),a.replace(/(\d)(?=(\d{3})+(?!\d))/g,"$1,");console.warn("format type unknown: "+c)}if(moqui.isNumber(a))return a.toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g,"$1,");c=moment(a);return c.isValid()?(b&&0!==b.length||(b="YYYY-MM-DD HH:mm"),c.format(b)):a},debounce:function(a,b){var c,d;return function(){var e=this,f=arguments;clearTimeout(c);c=setTimeout(function(){c=
null;d=a.apply(e,f)},b);return d}},makeColumnsConsistent:function(a){a=$("#"+a+" table");var b=[],c,d;for(c=0;c<a.length;c++){var e=a[c];if(e.rows&&0!==e.rows.length){var f=0;for(d=e.rows[f];5>f&&f<e.rows.length;)if((!d.cells||1>=d.cells.length)&&e.rows.length>f+1)f++,d=e.rows[f];else break;if(d.cells&&0!==d.cells.length)for(e=0;e<d.cells.length;e++){var g=$(d.cells[e]).width();if(!b[e]||b[e]<g)b[e]=g}}}d=b.length;for(c=e=0;c<d;c++)e+=b[c];g=[];for(c=0;c<d;c++)g[c]=100*b[c]/e;for(c=0;c<a.length;c++)if(e=
a[c],e.rows&&0!==e.rows.length){f=0;for(d=e.rows[f];5>f&&f<e.rows.length;)if((!d.cells||1>=d.cells.length)&&e.rows.length>f+1)f++,d=e.rows[f];else break;if(d.cells&&0!==d.cells.length)for(e=0;e<d.cells.length;e++)d.cells[e].style.width=g[e]+"%"}},downloadData:function(a,b,c){a=new Blob([a],{type:c});if(window.navigator.msSaveOrOpenBlob)window.navigator.msSaveOrOpenBlob(a,b);else{var d=document.createElement("a"),e=URL.createObjectURL(a);d.href=e;d.download=b;document.body.appendChild(d);d.click();
setTimeout(function(){document.body.removeChild(d);window.URL.revokeObjectURL(e)},0)}},notifyNotification:function(a,b){if(a){var c={};a.topic&&a.topic.length&&(c.tag=a.topic);if(window.Notification&&"granted"===Notification.permission){var d=new Notification(a.title,c);a.link&&a.link.length&&(d.onclick=function(){window.open(a.link)});moqui.webrootVue&&moqui.webrootVue.addNotify(a.title,a.type)}else window.Notification&&"denied"!==Notification.permission?Notification.requestPermission(function(e){"granted"===
e?(e=new Notification(a.title,c),a.link&&a.link.length&&(e.onclick=function(){window.open(a.link)}),moqui.webrootVue&&moqui.webrootVue.addNotify(a.title,a.type)):b(a)}):b(a)}},NotifyOptions:function(a,b,c,d){this.message=moqui.htmlEncode(a);b&&(this.url=b);this.icon=d?d:"success"===c?"fa fa-check-circle":"warning"===c?"fa fa-exclamation-triangle":"danger"===c?"fa fa-exclamation-circle":"fa fa-info-circle"},NotifySettings:function(a){this.delay=4E3;this.offset={x:20,y:60};this.placement={from:"top",
align:"right"};this.animate={enter:"animated fadeInDown",exit:""};this.type=a?a:"info";this.template='\x3cdiv data-notify\x3d"container" class\x3d"notify-container col-xs-11 col-sm-3 alert alert-{0}" role\x3d"alert"\x3e\x3cbutton type\x3d"button" aria-hidden\x3d"true" class\x3d"close" data-notify\x3d"dismiss"\x3e\x26times;\x3c/button\x3e\x3cspan data-notify\x3d"icon"\x3e\x3c/span\x3e \x3cspan data-notify\x3d"message"\x3e{2}\x3c/span\x3e\x3ca href\x3d"{3}" target\x3d"{4}" data-notify\x3d"url"\x3e\x3c/a\x3e\x3c/div\x3e'},
